...
class DartImageBenchmark extends AsyncBenchmarkBase {
  ...
  @override
  Future<void> run() async {
    // Simuliamo lo stesso workload del transformer nativo: decodifica, resize (fit), encode
    final image = img.decodeImage(imageBytes);
    if (image == null) throw Exception('Cannot decode image');
    
    // Resize "fit box" fino a max 500x500
...
void main() async {
  ...
  logger.info('Sample image loaded: ${imageBytes.length} bytes.');
  
  logger.info('--- Starting Native Benchmark ---');
